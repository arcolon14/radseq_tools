...
To run \emph{RADseqTools}, make sure you have a recent \texttt{R} installation. The package was written using \texttt{R version 3.4.2}, but should work with versions above \texttt{3.1.0}. To verify the installed \texttt{R} version, use:
<<>>=
R.version.string
@
...
To begin, we need to load the reference sequence into a \texttt{RADseqTools} object. This new \texttt{sequence} object can then be used to search for restriction sites. Create a variable containing the path to the reference sequence.  
<<eval=FALSE>>=
